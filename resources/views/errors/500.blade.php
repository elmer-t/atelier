<x-public.error :title="__('Something went wrong')" :heading="__('Something went wrong on our end')" code="500">
    {{ __('This one is on us, not on you. The problem has been logged — please try again in a moment.') }}

    <x-slot:action>
        <a href="{{ route('home') }}"
           class="inline-flex items-center rounded-lg bg-zinc-900 px-4 py-2 text-sm font-medium text-white transition hover:bg-zinc-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200">
            {{ __('Go to the homepage') }}
        </a>
    </x-slot:action>
</x-public.error>
